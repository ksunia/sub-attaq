...
void LostState::onEntry(QEvent *)
{
    ...
    QString message = QString("Ты проиграл на уровне %1. Твой счет %2.").arg(game->currentLevel+1).arg(game->score);
...
void WinState::onEntry(QEvent *)
{
    ...
    if (scene->levelsData.size() - 1 != game->currentLevel) {
        message = QString("Ты прошел уровень %1. Твой счет %2.\nНажми пробел чтобы играть дальше.").arg(game->currentLevel+1).arg(game->score);
        //We increment the level number
        game->currentLevel++;
    } else {
        message = QString("Ты закончил игру на уровне %1. Твой счет %2.").arg(game->currentLevel+1).arg(game->score);
        //We set the level back to 0
        game->currentLevel = 0;
        //We set the score back to 0
        game->score = 0;
